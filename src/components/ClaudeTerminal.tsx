 ...
 *
 * On first mount it calls `find_claude_path` to locate the binary; if not
 * found it shows an install hint instead of a blank terminal.
 */
...
function getTermTheme(name: string): ITheme {
...
export function ClaudeTerminal({ visible }: ClaudeTerminalProps) {
  ...
  // Stable session id — generated once per component lifetime.
  ...
  // ── Initialise xterm + spawn PTY — fires once when visible & path known ───
  useEffect(() => {
    if (inited.current || !claudePath || !visible || !containerRef.current) return;
    inited.current = true;

    const term = new XTerm({
      theme:             getTermTheme(themeName),
      fontFamily:        "'JetBrains Mono', 'JetBrainsMono Nerd Font', 'FiraCode Nerd Font', monospace",
    ...
    term.attachCustomKeyEventHandler((e) => {
      ...
      if (meta && e.key === "v") {
        navigator.clipboard.readText().then((text) => {
          if (text) invoke("pty_write", { sessionId: sessionId.current, data: text }).catch(() => {});
        });
        return false;
      }
      if (meta && e.key === "k") {
    ...
    const ro = new ResizeObserver(() => {
      requestAnimationFrame(() => {
        ...
        invoke("pty_resize", { sessionId: sessionId.current, rows, cols }).catch(() => {});
      });
    });
    ro.observe(containerRef.current);
    roRef.current = ro;

    // PTY output → xterm (cancellation flag prevents duplicate listeners on remount)
    let listenCancelled = false;
    listen<string>(`pty-output-${sessionId.current}`, (ev) =>
    ...
    // Fit then spawn — wait for a real layout pass so dimensions are non-zero
  ...
  // ── Refit + focus when visibility or container size changes ───────────────
  useEffect(() => {
  ...
  }, [termSettings.fontSize, termSettings.lineHeight]);
  ...
  return (
    ...
    >
      ...
      {/* ── xterm container — always in DOM once component mounts ────────── */}
      <div
        ref={containerRef}
